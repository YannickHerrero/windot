; Amphetamine - Natural Mouse Movement Script
; Moves mouse 20px with natural movement every 4 minutes
; Toggle with Ctrl+Alt+A

global amphetamineEnabled := true

; Start timer on load (enabled by default)
SetTimer(NaturalMouseMove, 240000)

; Toggle amphetamine on/off
ToggleAmphetamine() {
    global amphetamineEnabled

    if (amphetamineEnabled) {
        SetTimer(NaturalMouseMove, 0)  ; Stop timer
        amphetamineEnabled := false
        ToolTip("Amphetamine: OFF")
    } else {
        SetTimer(NaturalMouseMove, 240000)  ; Start timer
        amphetamineEnabled := true
        ToolTip("Amphetamine: ON")
    }
